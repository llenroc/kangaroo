...
namespace Hidistro.SqlDal.Members
{
	public class DistributorsDao
	{
        ...
        public DbQueryResult GetMilkCards(MilkCardQuery query)
        {
            ...
            {
                case 0://未发放
                    stringBuilder.Append(" AND sm.status =0 and sm.userId  is  null");
                    break;
                case 1://发放已使用
                    stringBuilder.Append(" AND sm.status =1 and sm.UserId>0");
                    break;
                case 2://已发放未使用
                    stringBuilder.Append(" AND sm.status =0  and sm.UserId>0");
                    break;
        ...
        public int SendMilkCards(string[] openids,string[] cardids)
        {
            ...
            for (int i = 0; i < cardids.Length && i < openids.Length; i++){
                stringBuilder.Append(string.Format("update site_milkcard set userid = (select userid from aspnet_members where userid = '{0}') where id = '{1}' ;", openids[i],cardids[i]));
            }
        ...
        {
            bool result;
            string query = "UPDATE Site_QuestList set OperaterTime=getdate(),Status=1,OperaterId=" + opid+"  WHERE QuestId in (" + ids + ") and   getdate() > QuestDate ";
            System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
            result = (this.database.ExecuteNonQuery(sqlStringCommand) > 0);


            //送了一次之后这个订单就是已发货状态
            string query1 = "UPDATE Hishop_Orders set OrderStatus=5 where OrderId in  (select OrderId from  Site_QuestList WHERE QuestId in (" + ids + ")) ";
             sqlStringCommand = this.database.GetSqlStringCommand(query1);
           int count= this.database.ExecuteNonQuery(sqlStringCommand);
