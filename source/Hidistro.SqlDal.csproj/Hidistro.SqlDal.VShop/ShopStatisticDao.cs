	...
	public class ShopStatisticDao
	{
		...
		public System.Data.DataTable ShopGlobal_GetSortList_Distributor(DateTime BeginDate, int TopCount)
		{
           StringBuilder stringBuilder = new StringBuilder();
            ...
            stringBuilder.AppendLine("  select a.*,b.StoreName,ROW_NUMBER() OVER(ORDER BY asordertotal DESC) AS rownum from");
            stringBuilder.AppendLine(" (select  count(*) as Ordernums, sum(ordertotal) asordertotal, SiteId from Hishop_Orders where OrderDate >= '" + BeginDate.ToString("yyyy-MM-dd") + "' group by SiteId)a");
            stringBuilder.AppendLine(" inner join aspnet_Distributors b on a.SiteId = b.UserId");
