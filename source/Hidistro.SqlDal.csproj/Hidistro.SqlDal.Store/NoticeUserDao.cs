...
namespace Hidistro.SqlDal.Store
{
	public class NoticeUserDao
	{
		...
		public void AddUser(int userid, string adminname, string fromPage = "")
		{
            string query = "";
            query += string.Format("delete from Hishop_NoticeTempUser where LoginName=@LoginName and UserID={0}",userid);
            query += string.Format(";insert into Hishop_NoticeTempUser(UserID,LoginName {0}) values ({1},@LoginName {2})", string.IsNullOrEmpty(fromPage) ? "" : ",FromPage", userid, string.IsNullOrEmpty(fromPage) ? "" : ",'" + fromPage + "'");
