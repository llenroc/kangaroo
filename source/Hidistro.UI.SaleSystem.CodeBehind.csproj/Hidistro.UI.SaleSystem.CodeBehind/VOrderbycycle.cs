		...
		protected override void AttachChildControls()
		{
            ...
            if(orderinfo != null)
            {
                foreach (LineItemInfo iteminfo in orderinfo.LineItems.Values)
                {
                    ...
                    //续订的第一天为之前订单的最后一天+1
                    this.hidSendStartDate.Value = iteminfo.SendEndDate.AddDays(1).ToString("yyyy-MM-dd");// iteminfo.SendStartDate.ToString("yyyy-MM-dd");
                    this.hidSendEndDate.Value = iteminfo.SendEndDate.AddDays(iteminfo.SendDays).ToString("yyyy-MM-dd"); //iteminfo.SendEndDate.ToString("yyyy-MM-dd");
                }
            }
