...
namespace Hidistro.UI.Web.Admin.Trade
{
	...
	public class OrderDetails : AdminPage
	{
        ...
        protected System.Web.UI.WebControls.Literal litTime;
	...
	//	protected System.Web.UI.HtmlControls.HtmlInputButton btnViewLogistic;
        ...
        protected System.Web.UI.WebControls.Literal litOrerTotal;
         protected System.Web.UI.WebControls.Literal litDays;
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (this.order != null)
			{
				if (!base.IsPostBack)
				{
                    ...
                    this.litOrerTotal.Text = Convert.ToDouble( OrderHelper.GetOrdersAndLines(this.orderId).Tables[0].Rows[0]["OrderTotal"]).ToString("0.00")+"元";
                    DataTable dtItem = OrderHelper.GetOrdersAndLines(this.orderId).Tables[1];
                    litDays.Text = dtItem.Rows[0]["SendDays"].ToString();
                    this.litTime.Text = dtItem.Rows[0]["SendStartDate"].ToString().Replace(" 0:00:00", "").Replace("/", "-") + "至" + dtItem.Rows[0]["SendEndDate"].ToString().Replace(" 0:00:00", "").Replace("/", "-");
                    ...
                    ///临时修改
                    if (!string.IsNullOrEmpty(this.order.PaymentType))
                    {
                        this.litPayType.Text = this.order.PaymentType;
                    } else
                    {
                        this.litPayType.Text = "奶卡支付";
                    }
					...
					this.litShippingRegion.Text = this.order.ShippingRegion+" "+this.order.Address;
